...
import { i } from '@css-bookends/css-calipers';
import { describe, expect, it } from 'vitest';

import { publishCompendium } from '../src/index';

describe('compendium config cascade -> calipers primitives', () => {
  it('compendium.global reaches the primitives (warn)', () => {
    const c = publishCompendium({ global: { hardening: 'warn' } });
  ...
  it('the nested calipers key configures a primitive (integer)', () => {
    const c = publishCompendium({
      calipers: { integer: { hardening: 'warn' } },
    });
    expect(c.i(8, { min: 0, max: 10 }).multiply(2).value()).toBe(16);
  });
  ...
  it('codex global (calipers.global) overrides compendium global', () => {
    const c = publishCompendium({
      ...
      calipers: { global: { hardening: 'warn' } },
    });
    expect(c.i(8, { min: 0, max: 10 }).multiply(2).value()).toBe(16);
  });
