import type { SelectorMap, StyleRule, StyleWithSelectors } from "../types";
...
export const outputVanillaExtract = (
  media: StyleRule
): SelectorMap =>
  mediaQueryOutputVanillaExtract<SelectorMap>(media);

export const preprocessorVanillaExtract = (media: StyleRule): SelectorMap => {
  ...
  const collectSelectors = (
    ...
    Object.keys(selectors).forEach((selector) => {
      ...
      collectSelectors(selectorRule as StyleWithSelectors, combined, acc);
    });
  };

  Object.keys(mediaQueries).forEach((query) => {
    const rule = mediaQueries[query] as StyleWithSelectors | undefined;
    if (!rule) return;
    ...
    Object.keys(perQuery).forEach((selector) => {
      ...
      if (!selectorRule || Object.keys(selectorRule).length === 0) return;
