 ...
 * obvious: `createIntegerFactory({ min, max })` binds a factory once, destructure-rename its `i` to a name
...
const { i: intFrom0To10 } = createIntegerFactory({ min: 0, max: 10 });
...
const { f: floatFrom0To1 } = createFloatFactory({ min: 0, max: 1 });
