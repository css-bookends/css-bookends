...
- **NOT provable at compile:** floats (a fraction has no type-level integer encoding, so opacity, line-height, and any fractional token are exempt), NEGATIVE values or bounds (tuple arithmetic has no negative length, so a negative `z-index` or `order` is exempt), non-literal values or factors (`i(someVariable)`), and chains whose running product tops ~10,000 (it compounds quickly, `400 * 3 * 3` already overshoots).
