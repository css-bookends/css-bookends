...
**A measurement's bound rides on its scalar (config-driven).** `m` and `r` are containers: they embed a scalar and carry NO numeric config of their own. To bound a measurement, bound the `i` / `f` you hand it, `m(i(v, { min, max }), 'px')`, and `m` surfaces that scalar's `.constraints()`. The bound (`min` / `max`) and the intake `modifier` both live on the `i` / `f` (see Integers and floats), never on `m` or a unit helper. When later arithmetic BREAKS the bound, the operation THROWS: a bound you set is a bound you enforce, there is no reaction knob. If you do not want enforcement, use a plain number (the bare `u`, no bound); the planned `snap` config will absorb a breach to the limit instead of throwing. A plain-number measurement, `m(5)`, wraps the bare `u` scalar: no bound, so it never reacts. Keep the simple path simple; reach for `i` / `f` only when you need the checks.
