...
- **Provable at compile (a red squiggle):** an INTEGER math op — `multiply`, `add`, `subtract`, or `divide` — on a FULLY BOUNDED value (both `min` and `max` set, so the value is captured), where the value, the LITERAL operand, and each bound are non-negative and under ~1,000 (three digits, TypeScript's per-tuple recursion limit). The result may be ANY size: the comparison short-circuits at the bound, so even a large overflow is caught without hitting the ~10,000 tuple wall. For example `i(5, { min: 0, max: 10 }).multiply(3)` is a type error (`15 > 10`), as is `.add(8)` (`13 > 10`) or `i(2, { min: 0, max: 10 }).subtract(5)` (`-3 < 0`). `multiply` and `add` can only breach the max; `subtract` and `divide` only the min (dividing shrinks toward zero). A single-edge bound (`{ max: 10 }` alone) keeps its runtime check but gets no squiggle.
- **NOT provable at compile:** floats (a fraction has no type-level integer encoding, so opacity, line-height, and any fractional token are exempt), NEGATIVE values or bounds (tuple arithmetic has no negative length, so a negative `z-index` or `order` is exempt), any value / operand / bound of ~1,000 or more (four digits, past the per-tuple recursion limit), and non-literal values or operands (`i(someVariable)`, or a plain runtime `number`).
