...
`m()` is generic over the unit (`m<Unit extends string>`), so ANY number-plus-unit is an `m()`, the named helpers (`mPx`, `mDeg`, …) are shortcuts, not the limit. Constraints (alpha `[0,1]`, counts `>= 1`, …) live on the scalars (`i` / `f`); a measurement carries a bound by embedding a bounded scalar. Together that closes the exact space csstype leaves open (`(number & {})` / `(string & {})`); string / identifier / url tokens stay csstype's `string`. The full proof, with every primitive CSS value type mapped to its lexicon, is [`docs/input-coverage.md`](./docs/input-coverage.md).
...
`i()` (a whole number) and `f()` (a real number) are constrained scalars for the unitless number space CSS leaves untyped. Both validate at construction and re-validate on every operation, so a constrained value stays valid (or throws) through arithmetic. `clamp(min, max)` snaps into range instead of throwing; a per-value bound (`i(v, { min, max })`) or a named-domain factory bound (`const { i: fontWeight } = createIntegerFactory({ min: 100, max: 900 })`) sets range constraints. The `createIntegerFactory` / `createFloatFactory` factories bake an optional bound into the `i` / `f` they return (a breach throws, or absorbs to the limit under an opt-in `snap`). An optional `modifier` (a `'floor'` / `'ceil'` / `'round'` shortcut or a function) transforms the value at intake, before the integer check, so an `i` can round or snap to a grid (e.g. font weights to multiples of 100) while still validating as a whole number; the opt-in `warnOnNonIntegerInput` flags a non-integer the modifier had to clean up. The combined scalar reference (`i` / `f`, the internal unspecified `u`, config, and brands) is [`docs/scalars.md`](./docs/scalars.md). See `examples/integers-floats.example.ts` and `examples/integer-modifier.example.ts`, and [`docs/value-modifier.md`](./docs/value-modifier.md).
...
**A measurement's bound rides on its scalar (config-driven).** `m` and `r` are containers: they embed a scalar and carry NO numeric config of their own. To bound a measurement, bound the `i` / `f` you hand it, `m(i(v, { min, max }), 'px')`, and `m` surfaces that scalar's `.constraints()`. The bound (`min` / `max`) and the intake `modifier` both live on the `i` / `f` (see Integers and floats), never on `m` or a unit helper. When later arithmetic BREAKS the bound, the operation THROWS by default; opt an edge into `snap` (`max: { value, snap: true }`, or a blanket `snap: true`) and a breach ABSORBS to the limit instead, silently (per-edge, so `min` and `max` react independently). A bound you set is a bound you enforce, unless that edge snaps. If you do not want enforcement at all, use a plain number (the bare `u`, no bound). See [`examples/snap.example.ts`](./examples/snap.example.ts). A plain-number measurement, `m(5)`, wraps the bare `u` scalar: no bound, so it never reacts. Keep the simple path simple; reach for `i` / `f` only when you need the checks.

```ts
import { m, i } from '@css-bookends/css-calipers';

// bound the SCALAR you hand m; the measurement surfaces that bound
const bounded = i(8, { min: 0, max: 10 });
m(bounded, 'px').constraints();          // { min: 0, max: 10 }
m(i(50, { min: 0, max: 10 }), 'px');     // throws (out of range)
...
- **Provable at compile (a red squiggle):** an INTEGER math op — `multiply`, `add`, `subtract`, or `divide` — on a FULLY BOUNDED value (both `min` and `max` set, so the value is captured), where the value, the OPERAND (a literal, or a bounded integer's own captured value), and each bound are non-negative and under ~1,000 (three digits, TypeScript's per-tuple recursion limit). The result may be ANY size: the comparison short-circuits at the bound, so even a large overflow is caught without hitting the ~10,000 tuple wall. For example `i(5, { min: 0, max: 10 }).multiply(3)` is a type error (`15 > 10`) — as is `.multiply(i(3, { min: 0, max: 5 }))`, which reads the operand's captured value `3` (its own bound is irrelevant) — and so are `.add(8)` (`13 > 10`) and `i(2, { min: 0, max: 10 }).subtract(5)` (`-3 < 0`). `multiply` and `add` can only breach the max; `subtract` and `divide` only the min (dividing shrinks toward zero). A single-edge bound (`{ max: 10 }` alone) keeps its runtime check but gets no squiggle.
- **NOT provable at compile:** floats (a fraction has no type-level integer encoding, so opacity, line-height, and any fractional token are exempt — a bounded `f` operand captures no value either, so it skips), NEGATIVE values or bounds (tuple arithmetic has no negative length, so a negative `z-index` or `order` is exempt), any value / operand / bound of ~1,000 or more (four digits, past the per-tuple recursion limit), and non-literal or unbounded operands (`i(someVariable)`, an unbounded `i(3)`, or a plain runtime `number`).
...
The bare exports (`m`, `color`, the refinements, ...) are each a factory already called at its defaults, so the default instance and a custom instance share one construction path. The factory itself is the real configurable path and the override seam: `createCalipersFactory({ errorConfig?, defaultUnit? })` returns a measurement instance (a container, no numeric config), `createIntegerFactory({ errorConfig? })` / `createFloatFactory({ errorConfig? })` return the config-bearing integer / float surface, and `createColorFactory({ formats?, output?, strictness?, transparent? })` a colour instance carrying its full config (custom format plugins, the default output format, strictness, transparency); routing every consumer through a factory means you can rewrite or wrap any step (input, storage, output) with zero call-site changes. The master factory `createCalipersBundleFactory({ global?, measurement?, integer?, float?, ratio?, color? })` (surfaced on the package root, and the default export of the `codex` entry) combines all of the above under one keyed config: a `global` slot of shared options plus one key per unit, where each setting resolves own key -> `global` -> factory default. So `createCalipersBundleFactory({ global: { errorConfig: { stackHints: 'off' } }, integer: { errorConfig: { stackHints: 'on' } } })` gives integer errors a verbose stack trace and everything else a terse one (the config cascades to the scalar family; `m` / `r` are containers and inherit via the scalar they embed). `codex` is the lazy-defaults convenience entry: it default-exports that master factory and named-exports the same primitive set already bound at defaults, so you get everything wired without touching a factory yourself. (`codex` and the bookends `@css-bookends/compendium/defaults` subpath are the only two lazy-defaults entries in the project.) Binding a factory once in one of your own modules and re-exporting the bound helper from there also keeps the blast radius of a library change small: your code imports that helper from a single seam, so a major restructuring of the library's internal paths lands in that one file, not across the hundreds or thousands of call sites that use it. And because each factory call returns its own independent instance, several configurations coexist side by side with no shared global state to collide: a `createColorFactory` that emits hex and another that emits oklch, or two integer factories with different bounds, all live at once, with no cascade or global state to fight (each instance is just a value in scope, not a stylesheet competing in the cascade). See the `examples/bind-once/` mini project (bind once, export, consume) and `examples/factory-wrapper.example.ts` (the override seam), and the full rationale in [the factory-first pattern](https://github.com/css-bookends/css-bookends/blob/main/docs/factory-first-pattern.md).
