...
**A measurement's bound rides on its scalar (config-driven).** `m` and `r` are containers: they embed a scalar and carry NO numeric config of their own. To bound a measurement, bound the `i` / `f` you hand it, `m(i(v, { min, max }), 'px')`, and `m` surfaces that scalar's `.constraints()`. The bound (`min` / `max`), the intake `modifier`, and the hardening reaction all live on the `i` / `f` (see Integers and floats), never on `m` or a unit helper. What happens when later arithmetic BREAKS the bound is one config knob, `hardening: 'warn' | 'fail'` (default `'fail'`): `fail` throws, `warn` warns and drops the bound. (There is no silent "ignore": dropping a bound silently is the same as never bounding the value.) Set it per scalar via `createInteger({ hardening })` / `createFloat({ hardening })`, or across the whole bundle via the cascade (see Factories). A plain-number measurement, `m(5)`, wraps the bare `u` scalar: no bound, so it never reacts. Keep the simple path simple; reach for `i` / `f` only when you need the checks.
...
m(i(50, { min: 0, max: 10 }), 'px');     // throws (out of range; default 'fail')
m(bounded).multiply(2);                  // throws (16 breaks [0, 10]; default 'fail')

// configure the reaction on the scalar family via the bundle global
const lenient = createCalipersBundle({ global: { hardening: 'warn' } });
lenient.m(lenient.i(8, { min: 0, max: 10 })).multiply(2).css(); // '16px' (bound dropped, proceeds)
```
