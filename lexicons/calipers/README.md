...
- **Provable at compile (a red squiggle):** an INTEGER operation on a non-negative LITERAL value with a non-negative LITERAL factor, where each value / factor / bound is under ~1,000 (three digits, TypeScript's per-tuple recursion limit). The product itself may be ANY size: `multiply` short-circuits the comparison at the bound, so even a large overflow is caught without hitting the ~10,000 tuple wall. For example `i(5, { max: 10 }).multiply(3)` is a type error, since `15 > 10`.
- **NOT provable at compile:** floats (a fraction has no type-level integer encoding, so opacity, line-height, and any fractional token are exempt), NEGATIVE values or bounds (tuple arithmetic has no negative length, so a negative `z-index` or `order` is exempt), any value / factor / bound of ~1,000 or more (four digits, past the per-tuple recursion limit), and non-literal values or factors (`i(someVariable)`, or a plain runtime `number`).
