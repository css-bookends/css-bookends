...
describe('css-value layer: intended-behaviour coverage sweep', () => {
  ...
  describe('instance-wide outOfRange default', () => {
    ...
    it('clamp is a no-op for open-bound helpers (nothing to clamp to)', () => {
      const clamped = createCssValues({ outOfRange: 'clamp' });
      // flexGrow has only a min, so a large value stays untouched and a
      // below-min value still throws.
      expect(clamped.flexGrow(9).css()).toBe('9');
      expect(() => clamped.flexGrow(-1)).toThrow(/below the minimum/);
    });
  });
