 ...
 * `import { publishBookGridRowStart, span } from '@css-bookends/grid-row-start'`.
 */
...
/**
 * A grid-row-start input: a grid-line position. A nonzero `<integer>` line number
 * (negatives count from the end edge), `'auto'`, a `<custom-ident>` named line, or
 * a `span N` count built with {@link span}.
 */
export type GridRowStartInput = GridLineInput;
 ...
 */
export type GridRowStartConfig = Record<string, never>;

/* ---------- the resolved, navigable result ---------- */

/**
 * The result of calling a grid-row-start book. "Typed token in, plain CSS out":
 * render with `.css()`, typed against csstype's `Property.GridRowStart`, or read
 * the raw rendered string via `.value()`.
 */
export interface ResolvedGridRowStart {
  /** the grid line as a CSS value, typed against `Property.GridRowStart`. */
  css(): Property.GridRowStart;
  /** the raw rendered string backing this value. */
  value(): string;
}

/** A grid-row-start book: called with a grid-line position. */
export type GridRowStart = (
  input: GridRowStartInput,
