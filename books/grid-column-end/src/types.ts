 ...
 * `import { publishBookGridColumnEnd, span } from '@css-bookends/grid-column-end'`.
 */
...
/**
 * A grid-column-end input: a grid-line position. A nonzero `<integer>` line number
 * (negatives count from the end edge), `'auto'`, a `<custom-ident>` named line, or
 * a `span N` count built with {@link span}.
 */
export type GridColumnEndInput = GridLineInput;
 ...
 */
export type GridColumnEndConfig = Record<string, never>;

/* ---------- the resolved, navigable result ---------- */

/**
 * The result of calling a grid-column-end book. "Typed token in, plain CSS out":
 * render with `.css()`, typed against csstype's `Property.GridColumnEnd`, or read
 * the raw rendered string via `.value()`.
 */
export interface ResolvedGridColumnEnd {
  /** the grid line as a CSS value, typed against `Property.GridColumnEnd`. */
  css(): Property.GridColumnEnd;
  /** the raw rendered string backing this value. */
  value(): string;
}

/** A grid-column-end book: called with a grid-line position. */
export type GridColumnEnd = (
  input: GridColumnEndInput,
